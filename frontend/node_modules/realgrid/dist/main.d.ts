...
export declare interface ColumnHeader extends ColumnObject {
     ...
     * ellipsis상태인 셀의 툴팁 표시여부
     *
     * `true`로 설정하면 grid.header.tooltipEllipsisOnly를 설정하지 않아도 ellipsis상태인 셀에 tooltip이 표시된다.
     *
     * @defaultValue `undefined`
     */
    tooltipEllipsisOnly?: boolean;
    ...
    templateCallback?: RealizeHeaderCallback;
    /**
     * {@link ColumnObject.template}으로 추가한 element에 event를 설정할때 사용
     *
     */
    templateEvents?: HeaderTemplateEvent | HeaderTemplateEvent[];
}
...
export declare interface ColumnObject {
     ...
     * {@link TemplateCellRenderer} 의 설명을 참고
     */
    templateEvents?: TemplateEvent | TemplateEvent[];
    /**
     * template에서 사용되는 값들
     */
    values?: any;
...
export declare interface ColumnStyleObject {
     ...
     *
     * @remarks
     * {@link DataColumn.lookupDisplay} 또는 {@link DataColumn.displayCallback} 보다 먼저 적용된다.
     */
    text?: string;
}
...
export declare interface ColumnSummary extends ColumnObject {
     ...
     *
     */
    templateEvents?: SummaryTemplateEvent | SummaryTemplateEvent[];
    /**
     * @readonly
     * {@link ColumnSummary.expression} 또는 {@link ColumnSummary.valueCallback}의 결과값을 가져올때 사용
     *
     * @remarks
     * 읽기만 가능한 값이다.
     *
...
export declare interface ColumnSummaryStyleObject {
     ...
     *
     * @remarks
     * text가 지정되면 {@link ColumnSummary.expression}과 {@link ColumnSummary.text} 속성은 무시된다.
     *
     */
    text?: string;
}
...
export declare interface CopyOptions {
    ...
    /**
     * 잘라내기를 할때 readOnly이거나 editable이 `false`인 컬럼의 값 초기화 여부
     *
     * @defaultValue `false`
     */
    cutReadOnlyCell?: boolean;
}
...
export declare abstract class CustomCellRendererImpl extends DataCellRenderer {
    ...
    get inputFocusable(): boolean;
    set inputFocusable(value: boolean);
    get dom(): HTMLElement;
    get itemClickable(): boolean;
    set itemClickable(value: boolean);
    /**
     * renderer의 type
     * @remarks
    ...
    /**
     * `true`이면 focus될 셀 또는 focus가 이동가능한 경우만 click이벤트가 발생한다.
     * {@link CellRenderer.clickOnlyFocusedCell}
     */
    get clickOnlyFocusedCell(): boolean;
    /**
     ...
     */
    protected abstract _doInitContent(dom: HTMLElement): void;
    /**
     ...
     *
     */
    protected abstract _doClearContent(dom: HTMLElement): void;
     ...
     *
     * @param grid - {@link GridBase}컨트롤
     * @param model - {@link GridCell} 객체
     * @param w - 너비
     * @param h - 높이
     */
    protected abstract render(grid: GridBase, model: GridCell, w: number, h: number, info: any): void;
    /**
     *
     * @param model - {@link GridCell} 객체
     * @param index - 셀의 위치정보 모델
     * @returns 반환된 값으로 tooltip이 보여진다.
     */
    tooltip(model: GridCell, index: CellIndex): string;
    /**
     * 편집 가능한 렌더러인지의 여부
     * @remarks
     * {@link CheckCellRenderer}와 같이 클릭에 의해서 값이 변경되는 renderer이면 `true`를 return한다.
     * @returns
     */
    canEdit(): boolean;
    /**
     ...
     *
     * @remarks
     * `false`를 return하면 그리드는 click이벤트를 처리하지 않고 element의 기본동작이 실행되도록 한다.
     *
     ...
     *
     * @remarks
     * `true`를 return하면 해당행의 상태를 편집중으로 변경한다.
     *
     * @param event - 마우스 클릭이벤트
     */
    canEditClickAt(event: MouseEvent): boolean;
    /**
     * 특정키가 down되었을때 `true`를 return하면 itemClick을 호출한다.
     *
     * @remarks
    ...
    /**
     * element를 클릭했을때 클릭가능한 element이면 {@link ClickData}를 반환한다.
     *
     * @remarks
     * 반환된 데이터는 {@link GridBase.onCellItemClicked}이벤트로 전달된다.
     *
     * `false`를 반환시 해당 이벤트를 중지시키고 {@link CustomCellRendererImpl.preventDefaultClick}을 호출한다.
     *
     * @param index - 셀의 위치정보 모델
     * @param event - 마우스 이벤트
     * @param target - 클릭된 element
     * @returns
     */
    click(index: CellIndex, event: Event, target?: HTMLElement): any;
    /**
     * {@link CustomCellRendererImpl.canClickSpaceKey}에서 `true`가 return되었을때 호출되는 method
     * @remarks
     ...
     *
     * @remarks
     * {@link GridBase.onCellItemClicked}에서 `false`를 return하면 호출된다.
     *
     * @param event - 마우스 이벤트
     */
    preventDefaultClick(event: Event): void;
    protected _doEditClick(index: CellIndex, event: MouseEvent, result: RendererEditResult): boolean;
}
...
export declare interface DataColumn extends ValueColumn {
     ...
     * @remarks
     *
     * `true`이면 그리드 편집기는 활성화되지만 편집은 할수 없다.
     *
     * @defaultValue `false`
     */
    readOnly?: boolean;
    ...
    breakMergeOnEmpty?: boolean;
    /**
     * 머지에 필요한 값을 사용자가 지정하기 위한 콜백.
     *
     * @eventProperty
     *
     * @remarks
     * 그리드는 return된 값을 이용해서 이전 행의 셀과 병합할 것인 지를 판단한다.
     */
     ...
     */
    filters?: ColumnFilterCollection;
    /**
     * 자동으로 생성된 filter들
     *
     * @remarks
     ...
     */
    validations?: EditValidationCollection;
    /**
 ...
 * ```
 */
export declare type DataDragCallback = (source: GridBase, sourceItems: number[], target: GridBase | HTMLElement, targetItem: number, targetColumn: string) => boolean;
 ...
 * ```
 */
export declare type DataDragLabelCallback = (source: GridBase, sourceItems: number[], target: GridBase | HTMLElement, targetItem: number, targetColumn: string) => string;
 ...
 * ```
 */
export declare type DataDropCallback = (source: GridBase, sourceItems: number[], target: GridBase | HTMLElement, targetItem: number, targetColumn: string) => void;
...
export declare interface DataDropOptions {
     ...
     * @defaultValue `false`
     */
    dropOtherElement?: boolean;
    /**
     * 다른 그리드로 drop여부
     *
     * @defaultValue `true`
     */
    dropOtherGrid?: boolean;
}
...
export declare interface DataOutputOptions {
     ...
     * 이 값이 지정되면 datetime 필드의 값을 출력할 때 변환 형식으로 사용된다.
     *
     * 표준시간대로 출력하려면 `yyyy-MM-dd HH:mm:ssZ`을 입력한다.
     *
     * 특정시간대로 출력하려면 `yyyy-MM-dd HH:mm:ss+` 또는 `yyyy-MM-dd HH:mm:ss+08:00`을 입력한다.
     *
     * 예) `'yyyy-MM-dd'`
     *
     */
...
export declare abstract class DataProviderBase {
    ...
    set valuesCallback(value: ObjectToValuesCallback | undefined);
    /**
     * 설정 정보를 반환한다.
     *
     * @remarks
     ...
     * @param resetFields - 데이터 컬럼에 binding된 필드 정보 갱신여부
     ...
     * 필드를 제거한다.
     *
     * @param field - 제거할 필드의 이름 또는 DataField 객체
     * @param resetFields - 데이터 컬럼에 binding된 필드 정보 갱신여부
     */
     ...
     */
    getDataRowId(dataRow: number): number;
...
export declare interface DateCellEditor extends TextCellEditor {
     ...
     *
     * @remarks
     * `false`로 지정하면 버튼이 표시되지 않고 날짜 또는 시/분/초를 선택시 list가 사라진다.
     *
     * @defaultValue `true`
     */
    showButtons?: boolean;
     ...
     * 시/분/초 선택화면 표시여부.
     *
     * @defaultValue TimeSelectType.NONE
     */
     ...
     * @defaultValue DateSelectView.DAYS
     */
 ...
 *     return value + "AA";
 * }
 * grid.columnByName("column").displayCallback = callback
 * ```
 */
declare type DisplayCallback = (grid: GridBase, index: CellIndex, value: any) => any;
...
export declare interface DisplayOptions {
    ...
    /**
     * 헤더의 너비를 변경할수 있는 마우스의 영역범위
     *
     * @defaultValue `3`
     */
    columnResizeThreshold?: number;
    /**
     * 컬럼 위치 변경 가능 여부
     *
     * @remarks
     ...
     *
     * @remarks
     * `true`로 설정하면 컬럼에 showTooltip을 설정하지 않아도 tooltip이 표시된다.
     * {@link CellRenderer.showTooltip | renderer.showTooltip}이 `false`인 경우 표시하지 않는다.
     *
     * @defaultValue `false`
     */
    showTooltip?: boolean;
     ...
     *
     * @remarks
     * `true`로 설정하면 ellipsis상태인 셀만 tooltip이 표시된다.
     * {@link CellRenderer.tooltipEllipsisOnly | renderer.tooltipEllipsisOnly}이 `false`인 경우 해당 컬럼은 모든 셀에 tooltip이 표시된다.
     *
     * @defaultValue `false`
     */
    tooltipEllipsisOnly?: boolean;
    ...
    /**
     * groupFocusVisible이 `true`일때 표시될 groupLayout을 지정한다.
     *
     * @remarks
     * `-1`: 최상위Layout이 선택
     * `0`보다 큰값이면 입력된 level에 해당하는 groupLayout이 선택된다.
     *
     * @defaultValue `-1`
     */
    ...
    /**
     * `true`이면 `altKey`를 누른상태에서 셀을 선택하면 drag&drop 모드로 변경한다.
     *
     * @remarks
     * {@link DisplayOptions.selectionStyle}이 `column`인 경우는 적용되지 않는다.
     *
     * @defaultValue `false`
     */
    altSelectAndImmediateDrag?: boolean;
    /**
    ...
    /**
     * selection 영역을 컨트롤 + 클릭으로 다시 클릭 시, selection 영역 제거 가능 여부
     *
     * @defaultValue `false`
     */
    selectionRemovable?: boolean;
    /**
     * 그리드가 편집중일때 tooltip 표시여부
     *
     * @remarks
     * `true`이면 편집중이 아닌 다른 셀에 mouse가 hover되면 tooltip이 표시된다.
     */
    showTooltipWhileEditing?: boolean;
}
...
export declare interface DropDownCellEditor extends TextCellEditor {
     ...
     *
     * @remarks
     * {@link SearchCellEditor} 같이 확장된 모델이 아닌, 기본 드롭 다운 편집기로 지정하고 싶다면 `'list'` 또는 `'dropdown'` 을 입력하면 된다.
     */
    type?: 'dropdown' | 'list';
    /**
     * 목록의 실제 값들
...
export declare interface EditOptions {
     ...
     * @remarks
     * {@link EditOptions.deletableWhenEdit}가 `false`인 경우 표시된다.
     *
     * @defaultValue `먼저 편집을 완료 하십시오.`
     */
    commitEditingMessage?: string;
    /**
    ...
    /**
     * cellButton 또는 renderer item을 클릭했을때 {@link GridBase.onCellClicked} 발생여부
     *
     * @remarks
     * `true`이면 `onCellClicked`이벤트가 발생하지 않는다.
     */
    exceptDataClickWhenButton?: boolean;
    /**
     ...
     * @defaultValue `false`
     */
    movableWhenFocusedOnly?: boolean;
}
...
export declare interface EditResult {
    ...
    oldValue: any;
    /**
     * {@link DropDownCellEditor.listCallback}에서 전달된 값
     */
    list?: any | any[];
}
 ...
 *
 * @remarks
 * {@link TreeOptions.expandWhenSetData} 에서 사용된다.
 */
export declare enum ExpandWhenSetData {
    /**
     * 펼침 상태
     */
    EXPAND = "expand",
    ...
    COLLAPSE = "collapse",
    /**
     * 펼침 상태 유지 및 신규행은 모두 펼친다.
     */
    REMAIN_AND_EXPAND = "remainAndExpand",
    /**
     * 펼침 상태 유지 및 신규행은 모두 접는다.
     */
    REMAIN_AND_COLLAPSE = "remainAndCollapse"
}
...
export declare interface ExportBaseOptions {
     ...
     *
     * @remarks
     * `STORE`를 사용해서 저장하는 경우 압축대비 10배정도의 크기를 가지지만 30% 정도의 속도 향상
     *
     * `STORE` 압축을 하지 않고 원본 그대로 저장
     * `DEFLATE` 압축을 해서 파일 size를 줄인다.
     * {@link https://stuk.github.io/jszip/documentation/api_jszip/generate_async.html#compression-and-compressionoptions-options | JSZip.compression} 참조
     */
     ...
     *
     * @remarks
     * `false`이면 압축된 파일을 메모리에 보관후 `crc32`를 파일의 앞쪽에 추가
     * `true`이면 파일의 마지막에 데이터 설명자 추가.
     * {@link https://stuk.github.io/jszip/documentation/api_jszip/generate_async.html#streamfiles-option | JSZip.streamFiles} 참조
     *
     * @defaultValue `true`
     */
    streamFiles?: boolean;
     ...
     *
     * @remarks
     * {@link ExportBaseOptions.compression} 이 `DEFLATE`일때 압축레벨
     *
     * `1` ~ `9`까지 정수로 입력한다.
     * {@link https://stuk.github.io/jszip/documentation/api_jszip/generate_async.html#compression-and-compressionoptions-options | JSZip.compressionOptions} 참조
     *
     * @defaultValue `6`
     */
     ...
     * 엑셀 파일의 속성을 지정할 수 있다.
     */
    coreProperties?: ExportCoreProperties;
    /**
...
/**
 * {@link ExportBaseOptions | 그리드 내보내기 설정}에서 엑셀파일의 속성을 지정 할 수 있는 속성
 */
export declare interface ExportCoreProperties {
    /**
     * 제목
     */
    title?: string;
 ...
 *
 * @remarks
 * {@link ExportMemo.author}을 사용하여 메모의 작성자를 설정할 수 있다.
 * {@link ExportMemo.validation}을 사용하여 유효성 검사 실패 셀의 오류 메시지를 메모로 내보낼지 여부를 설정하고,
 * {@link ExportMemo.callback}을 사용하여 각 셀의 메모를 출력할 때 실행할 콜백 함수를 정의할 수 있다.
 *
 * @example
 * ```js
 ...
 * ```
 */
export declare interface ExportMemo {
    /**
     *  메모에 작성자 설정
     *  {@link CellMemo.author}가 적용되어 있다면 무시된다.
     *
     *  @defaultValue " "
     */
    author?: string;
    /**
     * 메모에 작성될 작성자 표시 여부
     * {@link CellMemo.authorVisible}이 적용되어 있다면 무시된다.
     */
    authorVisible?: boolean;
    /**
     * 엑셀에 표시될 메모의 minWidth 설정 (단위: px)
...
export declare interface ExportOptions extends ExportBaseOptions {
    ...
    booleanFormat?: string;
    /**
     * 숫자 형식이면서 소수점이하가 `0`인 경우 적용되는 format
     *
     * @remarks
     *
     * {@link DataColumn.excelFormat}이 지정되거나 동적스타일에 `numberFormat`또는 `excelFormat`이 지정된경우에는 적용되지 않는다.
     */
    trailPointZeroFormat?: string;
    /**
 ...
 * [반환값] - 내보내기 될 값
 */
export declare type ExportValueCallback = (index: number, name: string, value: any, grid: GridBase | DataProviderBase) => any;
...
export declare interface FilterAutomatingOptions {
    ...
    /**
     * {@link DataColumn.autoFilter}에 의해서 만들어진 filter가 선택되었을때 호출되는 callback
     *
     * @remarks
     * filtering을 사용자가 결정할때 사용한다.
     */
    selectCallback?: FilterCallback;
    /**
     * 사용자가 추가한 필터도 함께 표시할때 사용한다.
     *
     * @defaultValue {@link UserFilterPosition.NONE}
     */
 ...
 *
 * @remarks
 * 반환값이 `undefined`인 경우 {@link ColumnFilter.criteria}가 적용된다.
 *
 * {@link ColumnFilter.callback} 또는 {@link FilterAutomatingOptions.selectCallback} 에서 사용된다.
 *
 * [매개변수 목록]
 *
 * `ds` - Provider 컨트롤
 ...
 * ```
 */
export declare type FilterCallback = (ds: LocalDataProvider | LocalTreeDataProvider, dataRow: number, level: number, field: number, filter: ColumnFilter, value: any) => boolean | undefined;
...
export declare interface FilteringOptions {
    ...
    toast?: ToastOptions;
    /**
     * 필터링 이후 유효한 행이 없다면 focus 제거 유무
     *
     * @defaultValue false;
     */
    clearFocusWhenFiltered?: boolean;
}
...
export declare interface FilterPanel {
    ...
    /**
     * filterPanel의 입력창에 text를 입력후 `enter`를 입력시 호출되는 callback
     *
     * @remarks
     * callback이 설정되면 그리드 filterPanel의 filtering은 적용되지 않는다.
     */
    filterPanelCallback?: FilterPanelCallback;
}

/**
 * @public
 * {@link FilterPanel}의 입력창에 text를 입력후 `enter`를 입력하거나 filterDelay이후 호출되는 callback이다.
 *
 * @remarks
 * `true`를 return하면 그리드의 filtering을 수행하지 않는다.
 * operator와 text를 return하면 입력된 operator와 text로 filtering을 한다.
 *
 * [매개변수 목록]
 *
 * grid - GridBase 컨트롤
 ...
 * column - 입력창의 컬럼 객체
 *
 * operator - {@link InlineFilterOperator}
 *
 * text - 입력된 text
 *
 * @example
 ...
 *
 */
export declare type FilterPanelCallback = (grid: GridBase, column: DataColumn, operator: InlineFilterOperator | string, text: string) => boolean | ConfigObject;
...
export declare interface FilterSelectorOptions {
    ...
    /**
     * `true`로 설정하면 filterResetText대신 image를 표시한다.
     *
     */
    useFilterResetImage?: boolean;
    /**
     ...
     *
     * @remarks
     * 필터선택 상자가 보여질때 category를 펼친상태로 보여준다.
     *
     * @defaultValue `false`
     */
    expandCategory?: boolean;
    /**
     * filter 선택창 상단에 표시되는 문자열
     *
     * @remarks
     * 지정하지 않으면 컬럼의 text가 표시된다.
     *
     * @defaultValue `undefiend`
     */
    title?: string;
    /**
     * filter 상단 title영역 표시여부
     *
     * @defaultValue `false`
     */
    showTitle?: boolean;
...
export declare abstract class GridBase {
     ...
     * ```
     */
    onValidateColumn?: (grid: GridBase, column: DataColumn, inserting: boolean, value: any, itemIndex: number, dataRow: number) => ValidationError | undefined;
    /**
     ...
     * ```
     */
    onContextMenuPopup?: (grid: GridBase, x: number, y: number, clickData: ClickData, menu: string, rightClick: boolean) => boolean | string;
    /**
     ...
     * @eventProperty
     */
    onShowEditCommand?: (grid: GridBase, index: CellIndex) => boolean;
    /**
    ...
    /**
     * 셀 편집이 완료되었을 때 셀의 위치와 편집결과를 갖고 있는 콜백
     *
     * @remarks
     * DropDownCellEditor, SearchCellEditor, MultiCheckCellEditor의 경우 editResult.text에는 labels에 해당하는 값이 출력되고 editResult.value에는 values에 해당하는 값이 출력된다.
     *
     ...
     * 다른 Editor의 경우 text와 value에 동일한 값이 출력된다.
     *
     * `boolean`을 return하면 {@link GridBase.onEditCommit}이 호출되지 않는다.
     *
     ...
     * ```
     */
    onGetEditValue?: (grid: GridBase, index: CellIndex, editResult: EditResult) => boolean | void;
    /**
     ...
     * ```
     */
    onCellPasting?: (grid: GridBase, index: CellIndex, value: any, editResult: EditResult) => boolean;
    /**
     ...
     *
     * `grid` - GridBase 컨트롤
     * `column` - filter Selector가 표시되는 경우 전달된다. filter의 active등을 변경하는 경우에는 전달되지 않는다.
     *
     ...
     * ```
     */
    onFiltering?: (grid: GridBase, column: DataColumn) => boolean;
    /**
     ...
     * ```
     */
    onFilteringChanged?: (grid: GridBase, column: GridColumn, filter: ColumnFilter, reset: boolean) => void;
    /**
     ...
     * ```
     */
    onShowTooltip?: (grid: GridBase, index: CellIndex, value: any) => boolean | string;
    /**
     ...
     * ```
     */
    onShowHeaderTooltip?: (grid: GridBase, column: GridColumn, value: any, layout: CellLayoutItem) => string;
    /**
     *
     * @deprecated
     ...
     * `event` - event객체
     */
    onGridFocusChanged?: (grid: GridBase, focused: boolean, event: Event) => void;
    /**
     ...
     * `event` - 이벤트 정보 ({@link https://developer.mozilla.org/en-US/docs/Web/API/ClipboardEvent| ClipboardEvent})
     *
     * [반환값] - `false`를 반환하면 잘라내기가 취소된다.
     * @defaultValue `null`
     * @eventProperty
     */
    onCut?: (grid: GridBase, selection: Selection | Selection[], event: ClipboardEvent) => boolean;
    /**
     * 그리드에 붙여넣기를 결정하는 콜백
     *
     * @remarks
     ...
     * ```
     */
    setCellStyleCallback(f: CellStyleCallback | undefined): void;
    /**
     * 행 스타일을 변경한다.
     *
     ...
     *
     * @remarks
     * 셀의 스타일을 지정하기 위한 규칙을 만들수 없는 경우 스타일을 적용하기 위해 사용한다.
     * column에 `-1`을 입력하면 행 전체에 적용된다.
     *
     *
     * @param dataRow - 행의 고유번호
     * @param column - 열의 이름
     * @param style - string 또는 {@link ColumnStyleObject}
     */
    addUserCellStyle(dataRow: number, column: string | -1, style: string | ColumnStyleObject): void;
    /**
     * addUserCellStyle로 지정한 스타일을 제거한다.
     *
     * @remarks
     * `null`을 입력하면 셀과 행에 적용된 스타일을 모두 제거한다.
     *
     * @param dataRow - 행의 고유번호
     * @param column - 열의 이름
     */
    removeUserCellStyle(dataRow?: number | null, column?: string | -1 | null): void;
    /**
     ...
     * @param column - 열의 이름
     */
    getUserCellStyle(dataRow: number, column: string | -1): string | ColumnStyleObject;
     ...
     * ```
     */
    setColumns(columns: (DataColumn | SeriesColumn | LiteralColumn | ConfigObject | string)[]): void;
    /**
     ...
     *
     * @param fieldName - 필드 이름 또는 index
     ...
     * ```
     */
    columnByField(fieldName: string | number): DataColumn;
    /**
     ...
     * ```
     */
    fitLayoutWidth(layout?: string | LayoutItem, maxWidth?: number, minWidth?: number, includeHeaderText?: boolean, includeFooterText?: boolean): number;
    /**
     * 행 높이를 지정한 높이로 변경한다.
     *
     *
     ...
     *
     * @remarks
     * 그리드 생성시 입력한 `div`요소의 top-left를 `0`으로 계산한다.
     * `coordinateByBody`를 `true`로 입력하면 body의 top-left를 `0`으로 계산한다.
     *
     * @param x - x 좌표
     * @param y - y 좌표
     * @param coordianteByBody - body의 top-left를 기준으로 좌표를 전달하는 경우 `true`로 설정한다.
     ...
     * ```
     */
    mouseToIndex(x: number, y: number, coordianteByBody?: boolean): ConfigObject;
    /**
     * 그리드에서 지정한 셀의 좌표값을 반환한다.
     *
     * @param itemIndex - 행의 순서
     * @param column - 컬럼 이름이나 컬럼 객체
     * @param outer - `false`이면 그리드 container를 기준으로하는 좌표값을 반환한다.
     ...
     * ```
     */
    getCellBounds(itemIndex: number, column: string | GridColumn, outer?: boolean): Rectangle;
    /**
     ...
     * @returns 셀의 편집가능 여부
     */
    getWritable(itemIndex: number, column: string): boolean;
    /**
     * 그리드 컬럼에 필터를 설정한다.
     *
     * @remarks
     ...
     *
     * @param column - filter Selector 가 표시 될 컬럼 객체 또는 컬럼명
     * @param searchText - 검색하려는 문자열을 입력
     * @returns filter Selector표시 여부
     */
    showFilterSelector(column: string, searchText?: string): boolean;
    /**
     * filter panel에 있는 input에 포커스가 된다.
     *
     * @param column - inline filter에 포커스를 줄 컬럼명
     *
     * @example
     * ```js
     ...
     * ```
     */
    inlineFilterFocus(column: string): void;
    /**
     ...
     * @param editing - 현재 편집중인 경우, 편집중인 값을 가지고 올지에 대한 유무
     * @returns 데이터 필드의 값
     * @example
     * ```js
     ...
     * ```
     */
    getValue(itemIndex: number, field: string | number, editing?: boolean): any;
    /**
     ...
     *
     * @remarks
     * contextMenu를 가져오는 경우 `gridContextMenu`를 입력한다.
     *
     * @param name - 메뉴 이름
     ...
     * @param eventFire - selection관련 이벤트를 발생시키지 않는다.
     *
     * @example
     * ```js
     * let sel = {
     ...
     * ```
     */
    setSelection(item: Selection, eventFire?: boolean): void;
    /**
     * 그리드에서 선택할 영역을 추가한다.
     *
     * @remarks
     ...
     * ```
     */
    addSelection(item: Selection, eventFire?: boolean): void;
    /**
     * 선택 영역을 모두 제거한다.
     *
     * @example
     ...
     * ```
     */
    copyToClipboard(selection: Selection, copy?: boolean): string;
    /**
     * 현재 편집기의 내용을 셀에 반영한다.
     *
     * @remarks
     ...
     * @param force - 기본값: `true`, `true`설정 시, {@link GridBase.onEditCommit | onEditCommit}이 호출되지 않는다.
     *
     * @returns commit이 완료되면 `true` 반환
     *
     * @example
     * ```js
     ...
     * ```
     */
    commitEditor(hideEditor?: boolean, force?: boolean): boolean;
    /**
     ...
     * @param items - templateCallback으로 전달할 추가정보 배열
     *
     * @example
     * ```js
     ...
     * ```
     */
    fillEditSearchItems(column: string | GridColumn, searchKey: string, values?: string[], labels?: string[], items?: ListResult): void;
    /**
     ...
     * @param columns - 검증을 수행할 컬럼의 이름 또는 배열을 지정하면 입력된 컬럼만 검증을 수행
     ...
     * ```
     */
    validateCells(itemIndices: number | number[] | null, visibleOnly?: boolean, pageAllItems?: boolean, columns?: string | string[]): object;
    /**
     ...
     *
     * @param itemIndex - 행의 고유 번호 입력하면 해당하는 행의 셀 정보만 삭제한다.
     * @param fieldName - 필드 이름 또는 index 입력하면 해당하는 셀 정보만 삭제한다.
     *
     * @example
     * ```js
     ...
     * ```
     */
    clearInvalidCells(itemIndex?: number, fieldName?: string | number): void;
    /**
     * 값 검증에 실패한 셀 정보들을 삭제한다.
     *
     * @remarks
     ...
     * ```js
     * gridView.validateCells();
     * gridView.clearInvalidCellsOfRow(0);
     * let invalids = gridView.getInvalidCellsOfRow(0); // null
     * ```
     */
    clearInvalidCellsOfRow(dataRow: number, fieldName?: string | number): void;
    /**
     * 값을 검증한 결과값을 사용자가 그리드에 추가한다.
     *
     * @remarks
     * 유효성 검증 결과를 사용자가 관리할때 사용한다.
     *
     * @example
     * ```js
     * // 그리드가 유효성검사를 진행하지 않도록 한다.
     * gridview.editOptions.validateOnEdited = false;
     * const validateFunc = (grid, itemIndex, dataRow, field, value) => {
     *      if (value === "fail") {
     *          return {level: "error", message: "유효한 값이 아닙니다."}
     *      }
     * }
     *
     * gridView.onCellEdited = (grid, itemIndex, dataRow, field) {
     *      const value = grid.getValue(itemIndex, field);
     *      const validate = validateFunc(grid, itemIndex, dataRow, field, value);
     *      if (validate) {
     *          grid.addInvalidCell(itemIndex, field, validate.error, validate.message)
     *      } else {
     *          grid.clearInvalidCells(itemIndex, field);
     *      }
     * }
     *
     * ```
     *
     * @param itemIndex - 행의 순서
     * @param fieldName - 필드 이름 또는 필드 순서
     * @param level - 에러 Level {@link ValidationLevel}
     * @param message - 표시할 메세지 내용
     */
    addInvalidCell(itemIndex: number, fieldName: string | number, level: ValidationLevel, message: string): void;
    /**
     * 값을 검증한 결과값을 사용자가 그리드에 추가한다.
     *
     * @remarks
     * 화면에 표시되지 않는 행의 유효성값을 추가할때 사용한다.
     *
     * @param dataRow - 행의 dataRow
     * @param fieldName - 필드 이름 또는 필드 순서
     * @param level - 에러 Level {@link ValidationLevel}
     * @param message - 표시할 메세지 내용
     */
    addInvalidCellOfRow(dataRow: number, fieldName: string | number, level: ValidationLevel, message: string): void;
    /**
    ...
    /**
     * 화면에 셀 또는 지정하는 위치에 메세지를 표시한다.
     *
     * @remarks
     * x, y를 `number`로 지정하면 그리드의 왼쪽상단에서 지정된 위치만큼 떨어진 위치에 표시한다.
     * x를 {@link CellIndex}형식으로 지정하면 해당셀의 왼쪽 하단에 표시된다.
     *
     * @param message - 화면에 표시될 메세지를 string 또는 {@link TooltipOptions} 형식으로 지정한다.
     * @param x - `x`축의 값을 `number`또는 {@link CellIndex}형식으로 지정한다.
     * @param y - `y`축의 값을 지정한다.
     *
     * @example
     * ```js
     * // 현재 선택된 셀의 왼쪽하단에 메세지를 표시
     ...
     *
     * // 지정된 시간이후 사라지는 메세지
     * gridView.showMessage({message: "자동으로 사라지는 메세지", hideDuration: 10000}, grid.getCurrent());
     * ```
     *
     */
    showMessage(message: string | TooltipOptions, x: number | CellIndex, y?: number): void;
    /**
     ...
     * @param sourceId - LookupSource의 id
     * @param keys - 찾을 키 값
     * @returns - fillLookupData로 설정한 data
     *
     */
    getLookupData(sourceId: string, keys: string | string[]): LookupDataKeyValues;
    /**
    ...
    /**
     * 해당 체크바가 indeterminate상태면 `true` 를 반환한다.
     *
     * @param itemIndex - 아이템의 인덱스
     *
     * @example
     ...
     * ```
     */
    isIndeterminateItem(itemIndex: number): boolean;
    /**
     ...
     * ```
     */
    isIndeterminateRow(row: number): boolean;
    /**
     * 행이 체크되어있는지 여부
     *
     * @param row - 행 고유 번호
     ...
     *
     * @param itemIndex - 아이템의 인덱스
     * @param indeterminate - 기본값: `true`, `true`: indeterminate 상태, `false`: indeterminate 상태 해제
     *
     * @example
     * ```js
     ...
     * ```
     */
    indeterminateItem(itemIndex: number, indeterminate?: boolean): void;
    /**
     * 지정된 아이템을 체크하거나 해제한다.
     *
     * @param itemIndex - 아이템의 인덱스
     ...
     * ```
     */
    indeterminateRow(row: number, indeterminate?: boolean): void;
    /**
     ...
     *
     * @param model - GridItem
     * @param checked - 기본값: `true`, `true`: 체크, `false`: 체크 해제
     * @param exclusive - 기본값: `false`, `true`: 체크된 다른아이템이 있는 경우 모두 해제하고 row로 입력된 아이템만 체크
     * @param checkEvent - 기본값: `true`, {@link GridBase.onItemChecked | onItemChecked()} 의 발생 여부
     */
    checkModel(model: GridItem, checked?: boolean, exclusive?: boolean, checkEvent?: boolean): void;
    /**
     * 지정된 GridItem이 체크되어있는지 여부
     *
     * @param model - GridItem
     */
    isCheckedModel(model: GridItem): boolean;
    /**
     * 지정된 GridItem의 체크 가능여부를 반환한다.
     * @param model - GridItem
     */
    isCheckableModel(model: GridItem): boolean;
    /**
     * 해당 모델의 indeterminate상태를 변경한다.
     *
     * @param model - GridItem
     * @param indeterminate - `indeterminate` 값을 입력한다.
     */
    indeterminateModel(model: GridItem, indeterminate: boolean): void;
    /**
     * 해당 체크바가 indeterminate상태면 `true` 를 반환한다.
     * @param model - GridItem
     */
    isIndeterminateModel(model: GridItem): boolean;
    /**
     ...
     * @returns filtering된 행들의 dataRow목록을 반환한다.
     *
     * @example
     * ```js
     ...
     * ```
     */
    getFilteredRows(sort?: boolean): number[];
    /**
     * 필드의 합계값을 반환한다.
     *
     * @remarks
...
export declare interface GridExportOptions extends ExportOptions {
    ...
    /**
     * 그리드가 행병합 그룹핑 상태이면서 {@link GridExportOptions.separateRows}가 `true`일때 GroupFooter또는 GroupSummary셀의 값 출력여부
     *
     * @defaultValue `false`
     */
    fillMergeFooterValue?: boolean;
    /**
     ...
     * {@link ExportBaseOptions."type" | 내보내기 종류}가 {@link ExportType.EXCEL | 엑셀}일 때만 사용 가능하다.
     *
     * `true`로 설정 시, 그룹의 마지막 행 아래에 요약 행이 표시된다.
     * `false`로 설정 시, 그룹의 첫 번째 행 위에 요약 행이 표시된다.
     *
     * @defaultValue `true`
     */
     ...
     * {@link ExportBaseOptions."type" | 내보내기 종류}가 {@link ExportType.EXCEL | 엑셀}일 때만 사용 가능하다.
     *
     * `true`로 설정 시, 그룹의 마지막 열 오른쪽에 요약 열이 표시된다.
     * `false`로 설정 시, 그룹의 첫 번째 열 왼쪽에 요약 열이 표시된다.
     *
     * @defaultValue `true`
     */
     ...
     *
     * @remarks
     * `true`면 ActualTargetBullt 렌더러와 Bar렌더러를 내보낼 때, 도형으로 내보낸다.
     *
     * @defaultValue `false`
     */
    exportShape?: boolean;
     ...
     *
     * @remarks
     * `true`이면 일괄 다운로드하고 `false`이면 하나씩 다운로드 한다.
     *
     * @defaultValue `true`
     */
    bulkImageDownload?: boolean;
    /**
     * 사용자 지정 출력
     *
     * @remarks
...
export declare interface GridItem {
     ...
     */
    getData?: (field: string | number) => any;
}
...
export declare class GridView extends GridBase {
     ...
     * ```
     */
    getDataSource(): LocalDataProvider;
    /**
     ...
     *
     * @remarks
     * 지정한 행이 row이거나 footer이면 행의 그룹을 접는다.
     * 그룹이 접혀있는 경우 상위 그룹을 접는다.
     *
     * @param itemIndex - 행의 인덱스
     * @param level - `-1`이면 선택된 행을 포함하는 그룹중 접혀있지 않은 그룹을 접는다. `1`이상의 값이 입력되면 입력된 level에 해당하는 그룹을 접는다.
     */
    collapse(itemIndex: number, level?: number): void;
    /**
     * 지정한 행의 그룹을 펼친다.
     *
     * @remarks
     ...
     *
     * @param itemIndex - 행의 인덱스
     * @param level - level을 지정하면 해당 level까지 펼친다.
     */
    expand(itemIndex: number, level?: number): void;
    /**
     * 루트 그룹을 펼친다.
     *
     * @remarks
    ...
    /**
     * GroupFooter의 {@link ColumnSummary.expression | expression} 또는 {@link ColumnSummary.valueCallback | valueCallback}에서 결과값을 가져온다.
     *
     * @remarks
     * numberFormat, prefix등이 적용되지 않은 값이다.
     * 선택된 footer가 여러개인경우 배열로 리턴된다.
     *
     * @param model - 아이템 모델 정보
     * @param column - 컬럼
     * @param summaryIndex - summary의 index
     * @returns - any
     */
     ...
     * itemIndex를 입력하지 않거나 -1을 입력하면 현재 선택된 행에 생성한다.
     *
     * @param itemIndex - 기본값: `-1`, 기준 데이터 행. 이 행 앞에 새로운 데이터행을 생성한다.
     * @param shift - 기본값: `false`, `true`: 데이터 행 뒤에 생성
     * @param data - 기본값: null, 신규행에 추가될 data
     * @returns 입력가능한 상태이면 신규행의 index를 return하고 입력가능한 상태가 아니면 `-1`을 return한다.
     *
     * @example
     * ```js
     ...
     * ```
     */
    beginInsertRow(itemIndex?: number, shift?: boolean, data?: DataValues): number;
    /**
     ...
     * ```
     */
    beginAppendRow(data?: DataValues): number;
    /**
     ...
     * @param force - `true`이면 merge된 셀이 아니어도 입력된 itemIndex를 포함하는 배열로 return한다. default `true`
     *
    ...
    /**
     * {@link DataColumn.mergeCallback}에 의해서 return되는 값이 변경될때 내부 merge정보를 갱신하도록 한다.
     *
     * @param refresh - `true`이면 내부 merge정보를 갱신후 그리드를 다시 그린다. default `true`
     */
    populateMerges(refresh?: boolean): void;
...
export declare enum HandleVisibility {
    ...
    HIDDEN = "hidden",
    /**
     * active상태이거나 호버링 된경우 표시
     *
     * @remarks
     * filterIcon에만 적용된다.
     */
    ACTIVE_OR_HOVERED = "activeOrHovered"
}
...
/**
 * {@link ColumnHeader} 또는 {@link CellLayoutGroupHeader}의 template 이벤트 설정정보
 *
 * @remarks
 * `renderCallback`은 header가 다시 그려질때 호출된다.
 *
 * [상위 클래스]
 *
 * {@link TemplateEvent}
 ...
 * header.template = `<input type='button'>`;
 * header.templateEvents = [{
 *      selector: 'input[type="button"]',
 *      event: {},
 *      renderCallback(grid, header, element) {
 *              element.value = header.displayText
 *      }
 * }]
 * ```
 */
export declare interface HeaderTemplateEvent extends TemplateEvent {
    /**
     * header가 다시 그려질때 호출되는 콜백
     *
     * @remarks
     * 수시로 호출되기 때문에 시간이 많이 걸리는 작업을 하는 경우 성능이 저하 될 수 있다.
     */
    renderCallback?: HeaderTemplateEventCallback;
}

/**
 * @public
 *
 * {@link ColumnHeader.templateEvents.renderCallback} 또는 {@link CellLayoutGroupHeader.templateEvents.renderCallback}에 사용되는 콜백 형식
 *
 * @remarks
 * 컬럼 헤더가 갱신될때 호출된다.
 *
 ...
 * `element` - HTMLElement
 *
 * `initialize` - element가 생성된 직후 호출되는 경우는 `true`이고 화면 갱신시 호출되는 경우 `false`이다.
 *
 * @example
 *
 * ```js
 * column.header.template = ``;
 * column.header.templateEvents = {
 *      selector: ``,
 *      event: {
 *          renderCallback(grid, header, element) {
 *          }
 ...
 * ```
 */
declare type HeaderTemplateEventCallback = (grid: GridBase, header: ColumnHeader | CellLayoutGroupHeader, element: HTMLElement, initialize: boolean) => void;
...
/**
 * @public
 * {@link DropDownCellEditor}의 목록을 만들때 사용하는 콜백
 * 조건에 따라 변경되는 item을 가져올때 사용한다.
 *
 * @remarks
 * {@link DataColumn.lookupDisplay}가 적용되기 위해서는 {@link DataColumn.labelField}를 이용해야 한다.
 *
 * 컬럼에 {@link DataColumn.lookupSourceId | lookupSource}가 연결된 상태에서 `undefined`가 return 되면 lookupSource에 있는 data가 표시된다.
 *
 * [매개 변수 목록]
 *
 * `grid` - gridView
 ...
 * `index` - CellIndex
 */
export declare type ListCallback = (grid: GridBase, index: CellIndex) => ListResult | undefined | Promise<ListResult | undefined>;

/**
 * {@link ListCallback} 또는 {@link GridBase.onSearchCellButtonClick} 의 return type
 * @remarks
 * IE11에서 ListCallback을 이용해서 목록을 만드는 경우 Promise polyfill을 추가해야 한다.
 */
export declare type ListResult = {
    /**
     * list중 value에 해당하는 key를 입력한다.
     *
     * @defaultValue `value`
     */
    value?: string;
    /**
     * list중 label에 해당하는 key를 입력한다.
     *
     * @defaultValue `label`
     */
    label?: string;
...
/**
 * @public
 * {@link DropDownCellEditor} 또는 {@link SearchCellEditor}에서 list에 표시되는 item을 사용자가 작성할때 사용하는 콜백
 *
 * @remarks
 * [매개변수 목록]
 *
 ...
 * `label` - label
 *
 * `item` - {@link DropDownCellEditor.listCallback} 또는 {@link GridBase.fillEditSearchItems}에서 입력한 정보
 *
 *
 * @example
 *
 ...
 * ```
 */
export declare type ListTemplateCallback = (grid: GridBase, index: CellIndex, dom: HTMLDivElement, value: string, label: string, item: any) => void;
 ...
 *
 * @remarks
 * `value`로 지정된 값이 화면에 표시된다.
 *
 * `type`속성에 `'literal'`을 설정하면 적용된다.
 *
 * {@link ValueColumn}을 상속한다.
 */
export declare interface LiteralColumn extends ValueColumn {
    /**
     * 컬럼의 type
     */
    type: "literal";
...
export declare class LocalDataProvider extends DataProviderBase {
     ...
     */
    /**
     * rowCount를 읽어오거나 설정한다. 설정하는 경우 {@link LocalDataProvider.setRowCount} 참조.
     */
    get rowCount(): number;
     ...
     * ```
     */
    onRowUpdating?: (provider: LocalDataProvider, row: number, oldValues: RowValues, newValues: RowValues) => boolean;
    /**
...
export declare class LocalTreeDataProvider extends DataProviderBase {
     ...
     * ```
     */
    onRowUpdating?: (provider: LocalTreeDataProvider, row: number, oldValues: RowValues, newValues: RowValues) => boolean;
    /**
     * 행 수정이 되었음을 알리는 콜백
     *
     * @remarks
     ...
     * @returns 성공적으로 추가하면 추가된 행 고유번호를, 실패하면 -1을 반환한다.
     */
    insertChildNode(row: number, index: number, data: RowObject, childrenProp?: string, iconField?: string, noState?: boolean): any;
    /**
     ...
     * ```
     */
    updateRow(row: number, values: DataValues, strict?: boolean): boolean;
    /**
     ...
     * ```
     */
    removeRow(row: number, recursive?: boolean): boolean;
    /**
     * 지정한 데이터 행들을 삭제한다.
     *
     * @remarks
     ...
     * ```
     */
    removeRows(rows: number[], recursive: boolean): boolean;
    /**
...
/**
 * {@link ExportMemo | 엑셀 노트 내보내기}에서 셀 별로 메모 출력 시 수행될 콜백을 지정하기 위한 형식
 *
 * @remarks
 * [매개변수 목록]
 *
 ...
 *
 * [반환값] - {@link CellMemo | CellMemo} | string | undefined
 *
 * @example
 * ```js
 * gridView.exportGrid({
 ...
 * ```
 */
export declare type MemoCallback = (grid: GridBase, itemIndex: number, dataRow: number, column: string, cellMemo: CellMemo) => CellMemo | string | undefined;
...
/**
 * {@link DataColumn.mergeCallback}에서 사용되는 콜백형식
 *
 * @remarks
 * 표시되는 값은 {@link DataColumn.displayCallback}을 이용해서 변경한다
 * `data`가 변경되는 경우에는 그리드가 다시 merge정보를 생성하지만 외부값에 따라 merge정보를 갱신해야 하는 경우 {@link GridView.populateMerges}를 이용해서 갱신한다.
 *
 * [매개변수 목록]
 *
 * `grid` - GridView
 ...
 *
 */
declare type MergeCallback = (grid: GridBase, column: DataColumn, itemIndex: number, value: any) => any;
...
export declare interface MobileOptions {
    ...
    /**
     * touch move를 했을때 dragging또는 swipe로 인지하도록 하는 범위
     *
     * @defaultValue `10`
     */
    tapThreshold?: number;
    ...
    scrollOnEditing?: ScrollOnEditing;
    /**
     * `false`이면 그리드 내부에서 스크롤이 발생했을때 외부로 전달하지 않는다.
     *
     * @remarks
     * 일부 모바일 기기의 경우 그리드 스크롤대신 body 영역이 스크롤되는것을 방지하기 위해 사용한다.
     *
     * @defaultValue `true`
     */
    scrollPropagate?: boolean;
    /**
     * `true`이면 Selection시 크기를 변경할수 있는 handle이 보여진다.
     *
     * @defaultValue `false`
     */
    showSelectionHandle?: boolean;
}
 ...
 *
 * @remarks
 * 셀에 저장되는 값의 각 value 를 구분하기 위한 구분자 지정은 편집기의 속성이 아닌 {@link ValueColumn.valueSeparator | column.valueSeparator} 에서 지정하며 기본값은 ',' 이다.
 *
 ...
 * ```
 */
export declare interface MultiCheckCellEditor extends Omit<DropDownCellEditor, "type"> {
    /**
 ...
 * @remarks
 *
 * `values` 또는 {@link DataColumn.values} 에 지정된 값들중 하나 또는 여러개를 선택한다.
 *
 * [상위 클래스]
 *
 * {@link CellRenderer}
 ...
 * ```
 */
export declare interface MultiCheckCellRenderer extends CellRenderer {
    /**
     * 종류: 다중 선택
     */
    type: "multicheck";
     ...
     *
     * @remarks
     * 단축키를 변경하는 경우 {@link MultiCheckCellRenderer.keyToValueCallback}을 사용한다.
     */
    useCtrlNumber?: boolean;
    /**
     ...
     *
     * @remarks
     * 콜백에서 `true`를 return 하면 disabled상태가 된다.
     *
     * @eventProperty
     */
    disabledCallback?: MultiCheckDisabledCallback;
    /**
     * keydown시 선택 또는 해제할 값을 가져오는 콜백
     *
     * @eventProperty
     */
    keyToValueCallback?: MultiCheckKeydownCallback;
}
 ...
 *
 * @remarks
 * {@link MultiCheckCellRenderer.disabledCallback} 에서 사용한다.
 *
 * [매개변수 목록]
 *
 * `grid` - GridBase 컨트롤
 ...
 * ```
 */
export declare type MultiCheckDisabledCallback = (grid: GridBase, itemIndex: number, column: DataColumn, value: any) => boolean;

/**
 * MultiCheckCellRenderer에서 keydown시 선택하거나 해제할 값을 결정하는 콜백 형식
 *
 * @remarks
 * {@link MultiCheckCellRenderer.keyToValueCallback} 에서 사용한다.
 *
 *
 * [매개변수 목록]
 *
 ...
 * ```
 */
export declare type MultiCheckKeydownCallback = (grid: GridBase, itemIndex: number, column: DataColumn, event: KeyboardEvent, values: string[]) => string | null;
...
export declare interface MultiLineCellEditor extends CellEditor {
     ...
     *
     * @remarks
     * `0`인경우 너비를 입력된 내용에 맞춰 자동으로 조절한다.
     * `-1`인 경우 셀의 너비로 고정한다.
     * `0` 이상인 경우 입력된 너비로 고정한다.
     *
     * @defaultValue `0`
     */
    width?: number;
 ...
 *
 * {@link SortingOptions.nullsOrder}에서 설정한다.
 */
 ...
 * @remarks
 *
 * javascript 표준 라이브러리를 이용해서 format을 수행한다.
 * {@link https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat/NumberFormat| Intl.NumberFormat}참조
 */
 ...
 *
 */
export declare type OuterSortCallback = (grid: GridBase, sortedFields: SortedFields[]) => void;
...
declare class RealGrid {
    ...
    /**
     * 브라우저의 확장 프로그램을 이용해서 IE모드로 그리드를 사용하는 경우 사용한다.
     *
     * @remarks
     * 확장프로그램의 IE모드에서는 `pointerEvent`대신 `mouseEvent`를 사용하도록 변경한다.
     * 그리드가 생성되기 전에 설정해야 한다.
     *
     * @param value - boolean
     */
    static setIECompatMode(value: boolean): void;
    /**
     * @public
     ...
     *
     * @remarks
     * filter, listEditor등이 그리드 내부에 생성되도록 한다.
     * drag&drop등 일부 기능을 사용할수 없다.
     *
     * @param value - boolean;
     */
    static setSlotMode(value: boolean): void;
    /**
     * 그리드에서 사용하는 메시지와 numberFormat등을 등록한다.
     * @param locale - locale객체
     */
    static setLocale(locale: RealGridLocale): void;
    /**
     * 등록되어있는 locale정보를 가져온다.
     *
     * @remarks
     * 지정되지 않은 경우 default로 적용되는 locale을 가져온다.
     *
     * @returns - RealGridLocale객체
     */
    static getLocale(): RealGridLocale;
    /**
     * 그리드 기본설정을 지정한다.
     * @remarks
     * {@link GridBase.setOptions}와 동일한 형태로 지정하면 이후 생성되는 그리드의 기본설정으로 적용된다.
     * @param options - option 객체
     * @param dataOptions - dataProvider option 객체
     */
    static setDefault(options: ViewOptions, dataOptions?: DataOptions): void;
    /**
     * @public
     ...
     * @param type - 사용자가 지정한 렌더러 종류 (이름)
     * @param renderer - {@link CustomCellRenderer} 객체
     * @param overwrite - `true`이면 기존 정의된 renderer를 대체한다.
     *
     * @example
     * ```js
     * // CustomCellRenderer 모델
     ...
     * ```
     */
    static registerCustomRenderer(type: string, renderer: any, overwrite?: boolean): void;
    static get CustomCellRendererImpl(): typeof CustomCellRendererImpl;
}
 ...
 *
 * numberFormats: {@link https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat/NumberFormat| Intl.NumberFormat}을 이용한 numberFormat을 등록한다.
 *
 * messages: 그리드의 메시지를 사용자 환경에 맞게 변경한다.
 *
 * @remarks
 * 화폐를 표시하기 위해서는 `currency`를 지정해야 한다.
 */
export declare interface RealGridLocale {
    /**
     * 언어 또는 언어-지역형태로 지정한다.
     */
    locale: string;
    ...
    currency: string;
    /**
     * 컬럼에서 사용할 numberFormat을 등록한다.
     * locale에 따라 천단위 구분기호와 소숫점 기호가 변경된다.
     *
     * @remarks
     * {@link https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat/NumberFormat|Intl.NumberFormat} 을 이용해서 화면에 출력될 문자열을 만든다.
     *
     * @example
     * ```js
     * RealGrid.setLocale({
     ...
     */
    numberFormats?: {
        [name: string]: NumberFormatOptions;
    };
    /**
     * 그리드에 표시되는 메시지를 지정한다.
     */
    messages?: RealGridMessages;
}
 ...
 * ```
 */
export declare interface RealGridMessages {
    /**
     * dateEditor에 표시되는 요일.
     */
    dateEditorWeekDays?: string[];
    /**
     * dateEditor의 월선택 화면에서 표시되는 월.
     */
    dateEditorMonths?: string[];
    /**
     * dateEditor의 저장버튼
     */
    dateEditorSaveLabel?: string;
    /**
     * dateEditor의 취소버튼
     */
    dateEditorCancelLabel?: string;
    /**
     * dateEditor의 삭제버튼
     */
    dateEditorDeleteLabel?: string;
    /**
     * dateEditor의 `Today`버튼
     */
    dateEditorTodayLabel?: string;
    /**
     * dateEditor의 `시`
     */
    dateEditorHourLabel?: string;
    /**
     * dateEditor의 `분`
     */
    dateEditorMinuteLabel?: string;
    /**
     * dateEditor의 `초`
     */
    dateEditorSecondLabel?: string;
    /**
     * dateEditor의 년을 표시할때 사용되는 format
     */
    dateEditorYearDisplayFormat?: string;
    /**
     * dateEditor의 월을 표시할때 사용되는 format
     */
    dateEditorMonthDisplayFormat?: string;
    /**
     * dateEditor의 년-월을 표시할때 사용되는 format
     */
    dateEditorYearMonthDisplayFormat?: string;
    /**
     * searchEditor에 `more`버튼
     */
    searchEditorMoreText?: string;
    /**
     * multiCheckEditor의 `확인`버튼
     */
    checkListAcceptText?: string;
    /**
     * multiCheckEditor의 `취소`버튼
     */
    checkListCancelText?: string;
    /**
     * multiCheckEditor의 `전체선택` 버튼
     */
    checkListAllCheckText?: string;
    /**
     * 필터 선택창의 `확인` 버튼
     */
    filterSelectorAcceptText?: string;
    /**
     * 필터 선택창의 `(전체 선택)`
     */
    filterSelectorAllCheckText?: string;
    /**
     * 필터 선택창의 `취소`버튼
     */
    filterSelectorCancelText?: string;
    /**
     * 필터 선택창의 `(값 없음)`
     */
    filterSelectorEmptyFilterText?: string;
    /**
     * 필터 선택창의 `초기화`
     */
    filterSelectorFilterResetText?: string;
    /**
     * 필터 선택창의 `전체 초기화`
     */
    filterSelectorFiltersResetText?: string;
    /**
     * 필터 선택창 검색 input의 placeHolder
     */
    filterSelectorSearchPlaceholder?: string;
    /**
     * 필터 선택창의 `(조회 전체 선택)`
     */
    filterSelectorSearchedCheckText?: string;
    /**
     * 필터 선택창 DateCategory사용시 년도에 대한 format
     */
    filterSelectorDateYearFormat?: string;
    /**
     * 필터 선택창 DateCategory사용시 분기에 대한 format
     */
    filterSelectorDateQuarterFormat?: string;
    /**
     * 필터 선택창 TimeCategory사용시 시간에 대한 format
     */
    filterSelectorHourFormat?: string;
    /**
     * 필터 선택창 TimeCategory사용시 분에 대한 format
     */
    filterSelectorMinuteFormat?: string;
    /**
     * 필터 선택창 TimeCategory사용시 초에 대한 format
     */
    filterSelectorSecondFormat?: string;
    /**
     * 필터 선택창 DateCategory사용시 월에 대한 format
     */
    filterSelectorDateMonthFormat?: string;
    /**
     * 필터 선택창 DateCategory사용시 날짜에 대한 format
     */
    filterSelectorDateDayFormat?: string;
    /**
     * filtering시 표시되는 toast창에 출력되는 문구
     */
    filterToastMessage?: string;
    /**
     * 그리드간 drag&drop시 출력되는 문자열 format
     */
    dataDropProxyLabel?: string;
    /**
     * dataDropOptions.dropMode가 `copy`일때 출력되는 메시지
     */
    dataDropModeCopy?: string;
    /**
     * dataDropOptions.dropMode가 `move`일때 출력되는 메시지
     */
    dataDropModeMove?: string;
    /**
     ...
     */
    groupingToastMessage?: string;
    /**
     * groupPanel 초기화버튼의 text
     */
    groupResetText?: string;
    /**
     * 그리드에 표시되는 data가 없는 경우 출력되는 메시지
     */
    displayEmptyMessage?: string;
    /**
     ...
     */
    invalidDatetimeFormat1?: string;
    /**
     * datetimeFormat 설정오류 메시지2
     */
    invalidDatetimeFormat2?: string;
    /**
     * 편집중인 행을 삭제시 표시되는 메시지
     */
    commitEditingMessage?: string;
     ...
     */
    rowIndicatorHeadText?: string;
    /**
     * rowIndicator headerSummary영역에 출력되는 문자
     */
    rowIndicatorSumText?: string;
    /**
     * rowIndicator footer영역에 출력되는 문자
     */
    rowIndicatorFootText?: string;
    /**
     * 신규행일때 stateBar에 출력되는 메시지
     */
    stateTextCreated?: string;
    /**
     * 편집된 행일때 stateBar에 출력되는 메시지
     */
    stateTextUpdated?: string;
    /**
     * 삭제된 행일때 stateBar에 출력되는 메시지
     */
    stateTextDeleted?: string;
    /**
     * 신규-삭제된 행일때 stateBar에 출력되는 메시지
     */
    stateTextCreateAndDeleted?: string;
    /**
     ...
     */
    fieldIndexOutOfBounds?: string;
    /**
     * 필드명이 누락되었을때 표시되는 메시지
     */
    fieldNameMustExists?: string;
    /**
     ...
     */
    clientEditingError?: string;
    /**
     * 그리드를 중복생성했을때 발생하는 error 메시지
     */
    gridElementAttachFail?: string;
    /**
     * 그리드 container를 찾지 못했을때 발생하는 error 메시지
     */
    gridContainerNotFind?: string;
}
 ...
 *
 * @remarks
 * {@link GridBase.getCellBounds}에서 셀의 위치정보를 반환하는 형식이다.
 */
...
export declare enum RefreshMode {
    ...
    /**
     * 세로스크롤시에는 보이는 컬럼만 다시 그리고 가로스크롤시에는 전체 컬럼을 다시 그린다.
     * @remarks
     * 가로스크롤이 연속되는 경우에는 다시 그리지 않고 위치만 이동한다.
     */
    RECYCLE = "recycle"
}
...
export declare interface RowGroup {
    ...
    mergeMode?: boolean;
    /**
     * mergeMode가 `true`일때 Grouping에서 해제된 컬럼이 원래의 위치로 이동되도록 한다.
     *
     * @remarks
     * grouping후 컬럼을 이동시키거나 또는 visible을 변경한 경우에는 정상적인 위치로 찾아가지 못한다.
     */
    resetOriginVindex?: boolean;
    /**
     * 자동 정렬 여부
     *
     * @remarks
     ...
     * @defaultValue `true`
     */
    indentVisible?: boolean;
    /**
     * 그룹 expander를 몇 레벨까지 보여질지 여부
     *
     * @remarks
     * {@link RowGroup.expandedAdornments}, {@link RowGroup.collapsedAdornments}가 {@link RowGroupAdornments.FOOTER} 또는 {@link RowGroupAdornments.SUMMARY} 인 경우만 적용된다.
     * `0` 전체 레벨이 보여진다.
     * 양수이거나 음수인경우 설정된 레벨까지 보여진다.
     * 그룹이 3레벨인경우 `1`로 설정하면 1레벨만 보여지고 `-1`로 설정하면 3레벨만 보여진다.
     *
     * @defaultValue `0`
     */
    indentVisibleLevel?: number;
    /**
     * expander 표시 여부
     ...
     *
     */
    footers?: RowGroupFooterCollection;
    /**
     * @deprecated
     * 로우그룹 푸터의 갯수와 style등을 지정한다.
     ...
     *
     * @remarks
     * `true`이면 그룹의 첫번째 데이터만 표시된다.
     * {@link RowGroup.mergeMode} 가 `true`일때 적용되지 않는다.
     *
     * @defaultValue `false`
     */
    blankGroupData?: boolean;
}
...
export declare interface RowIndicator {
    ...
    cellStyleCallback?: RowBarCellStyleCallback;
    /**
     * 선택된 행의 인디케이트 셀에서 행 이동 여부
     *
     * @defaultValue `false`
     */
    draggableSelectedRows?: boolean;
}
 ...
 * ```
 */
export declare type RowStyleCallback = (grid: GridBase, item: GridItem, fixed: boolean) => string | RowStyleObject | undefined;

/**
 * {@link RowStyleCallback} 으로 행의 스타일을 변경시 반환되는 스타일 모델
 *
 * @remarks
 ...
 * ```
 */
export declare interface SearchCellEditor extends Omit<DropDownCellEditor, "listCallback" | "type"> {
    /**
     * 셀 편집기 종류: 검색
     */
    type?: 'search';
    ...
    searchLength?: number;
    /**
     * 마지막 키 입력이 완료된 후 검색이 시작될때까지의 시간 (ms, 밀리세컨드)
     *
     * @defaultValue `1000`
     */
    searchDelay?: number;
 ...
 */
export declare interface SortedFields {
    fieldName: string;
    orgFieldName: string;
...
export declare interface SortingOptions {
     ...
     *
     * `true`로 지정하면 사용자가 grid.orderBy를 이용해서 정렬상태를 지정해야한다.
     *
     * `function`으로 지정하면 onSortingChanged이벤트 직전에 function을 호출해준다.
     *
     * @example
     * ```js
     ...
     *
     */
    outerSort?: OuterSortCallback | boolean;
    /**
...
export declare interface StateBar {
     ...
     *
     * @remarks
     * `true`면 focus 된 행에 해당하는 셀을 `'rg-statebar-focus'` 스타일로 표시한다.
     *
     * @defaultValue `false`
     */
    showFocus?: boolean;
     ...
     *
     * @remarks
     * `true`면 데이터 선택 영역에 맞춰 관련된 셀들을 `'rg-statebar-select'` 스타일로 표시한다.
     *
     * @defaultValue `false`
     */
    showSelection?: boolean;
...
/**
 * @public
 * {@link ColumnSummary.templateEvents.rendererCallback}에 사용되는 콜백 형식
 *
 * @remarks
 * 컬럼 푸터가 갱신될때 호출된다.
 *
 ...
 * @example
 * ```js
 * column.footer.template = `<input type='button'>`;
 * column.footer.templateEvents = [{
 *      selector: `input[type='button']`,
 *      event: {
 *          renderCallback(grid, column, summaryIndex, summary, element) {
 *          }
 *      }
 * }]
 * ```
 */
declare type SummaryTemplateCallback = (grid: GridBase, column: GridColumn, summaryIndex: number, summary: ColumnSummary | GroupItem, element: HTMLElement, initialize: boolean) => void;

/**
 * {@link ColumnSummary}의 template 이벤트 설정정보
 *
 * @remarks
 * `renderCallback`은 footer또는 headerSummary가 갱신될때 호출된다.
 *
 * [상위 클래스]
 *
 * {@link TemplateEvent}
 ...
 *
 */
export declare interface SummaryTemplateEvent extends TemplateEvent {
    /**
     * 수시로 호출되기 때문에 시간이 많이 걸리는 작업을 하는 경우 성능이 저하 될 수 있다.
     */
    renderCallback?: SummaryTemplateCallback;
}
...
/**
 * {@link TemplateCellRenderer} 또는 {@link ColumnObject.template}에서 생성되는 요소에 이벤트를 추가할때 사용되는 형식
 *
 * @remarks
 ...
 * ```
 */
export declare interface TemplateEvent {
    /**
     * 요소의 선택자
     *
     * @remarks
     * renderer의 하위 요소를 선택한다.
     * querySelect에 사용되는 선택자와 동일하게 사용한다.
     */
    selector: string;
    /**
     * 이벤트명과 실행될 method
 ...
 *
 * @remarks
 * {@link FilterAutomatingOptions.timeCategorize}에서 사용한다.
 */
export declare enum TimeCategory {
    /**
     * 시간으로 분류하지 않고 날짜로 분류한다.
     */
    NONE = "none",
    /**
     * 시간으로 분류한다.
...
export declare enum TimeSelectType {
    /**
     * 시간선택화면을 표시하지 않는다.
     */
    NONE = "none",
...
/**
 * 화면에 메세지를 표시하기 위한 설정 모델
 *
 * @remarks
 * 화면의 특정위치에 메세지를 표시할때 메세지와 사라지는 시간을 지정한다.
 * ```js
...
export declare interface TooltipOptions {
    ...
    message: string;
    /**
     * message가 표시되고 자동으로 사라지는 시간을 ms단위로 지정한다.
     */
    hideDuration?: number;
    /**
     * tooltip element의 class를 지정한다.
     */
    styleName?: string;
}
...
export declare interface TreeOptions {
    ...
    /**
     * softDeleting상태일때 상위 노드를 제거하면 하위 노드의 상태값을 {@link RowState.DELETED}로 변경한다.
     *
     * @defaultValue `false`
     */
    recursiveDeleting?: boolean;
    /**
     * set data 이후 기존 펼침 유지 여부를 설정할 수 있다.
     *
     * @remarks
     * {@link LocalTreeDataProvider.setRows} 또는 {@link LocalTreeDataProvider.setCsvRows}로 데이터를 설정할때에만 적용된다.
     */
    expandWhenSetData?: ExpandWhenSetData;
    /**
     * treeView의 첫번째 컬럼에 html tag를 표시할때 사용한다.
     */
    templateOptions?: TreeTemplateOptions | undefined;
}

/**
 * {@link TreeTemplateOptions}의 template 이벤트 설정정보
 *
 * @remarks
 * `renderCallback`은 TreeRenderer가 갱신될때 호출된다.
 *
 * [상위 클래스]
 *
 * {@link TemplateEvent}
 ...
 * @example
 * ```js
 * treeVeiw.treeOptions.templateOptions = {
 ...
 *
 */
export declare interface TreeTemplateEvent extends TemplateEvent {
    /**
     * TreeRenderer가 다시 그려질때 호출되는 콜백
     *
     * @remarks
     * 수시로 호출되기 때문에 시간이 많이 걸리는 작업을 하는 경우 성능이 저하 될 수 있다.
     */
    renderCallback?: TreeTemplateEventCallback;
}

/**
 * @public
 *
 * {@link TreeTemplateEvent.renderCallback}에 사용되는 콜백 형식
 *
 * @remarks
 ...
 * ```
 */
export declare type TreeTemplateEventCallback = (grid: GridBase, model: GridCell, element: HTMLElement, initialize: boolean) => void;

/**
 * treeView 첫번째 컬럼의 data영역에 html을 표시하기 위한 설정 정보
 *
 * @remarks
 * `input`, `textarea`와 같이 사용자 상호작용을 가지는 HTMLElement는 기본동작이 제한된다.
 *
 */
export declare interface TreeTemplateOptions {
    /**
     * 입력되는 html 템플릿 형태의 문자열
     *
     * @remarks
     ...
     * ```
     */
    templateEvents?: TreeTemplateEvent | TreeTemplateEvent[];
...
export declare class TreeView extends GridBase {
     ...
     */
    getDataSource(): LocalTreeDataProvider;
    /**
     ...
     *
     * @param level - 기본값: `0`, `0`일 때 recursive가 `true`면 모든 자손 펼침
     * @param renewHasChildren - 기본값: `true` `false`이면 자식행 여부를 갱신하지 않는다
     * @example
     *
     * ```js
     * tree.expandAll();
     * ```
     */
    expandAll(level?: number, renewHasChildren?: boolean): void;
    /**
     * 모든 행을 접는다.
     *
     * @example
 ...
 * 이미지의 경로와 위치를 지정한다.
 * @remarks
 * width, height가 지정되면 이미지의 원래 크기가 아닌 입력된 값을 이용해서 크기를 변경한다.
 */
declare interface UserCellImage {
    /**
     * 이미지의 url을 입력한다.
     */
    url: string;
    /**
     * 이미지의 위치를 지정한다.
     * @defaultValue IconLocation.CENTER
     */
    location?: IconLocation;
    /**
     * 이미지의 너비를 지정한다.
     * 지정하지 않으면 이미지의 원래너비를 사용한다.
     */
    width?: number;
    /**
     * 이미지의 높이를 지정한다.
     * 지정하지 않으면 이미지의 원래높이를 사용한다.
     */
    height?: number;
}
...
export declare interface UserExportCell {
     ...
     *
     * @remarks
     * 이미 있는 컬럼인 경우 기존너비가 유지된다.
     *
     * @defaultValue `undefined`
     */
    width?: number;
    /**
     * mergeCol이 있는 경우 각 컬럼의 너비를 지정한다.
     *
     * @remarks
     * 지정하지 않으면 width가 적용된다.
     */
    widths?: number[];
     ...
     *
     */
    image?: UserCellImage | string;
    /**
     * 셀에 메모를 출력한다.
     *
     * @remarks
     * string으로 입력하여 message를 설정할 수 있다.
     */
    cellMemo?: CellMemo | string;
}
 ...
 *
 */
export declare enum UserFilterPosition {
    /**
     * 사용자 필터를 autoFiler 상단에 위치시킨다.
     */
    FIRST = "first",
    /**
     * 사용자 필터를 autoFiler 하단에 위치시킨다.
     */
    LAST = "last",
    /**
     * 사용자 필터를 표시하지 않는다.
...
export declare interface ValueColumn extends GridColumn {
     ...
     * {@link CellEditor} 참조
     */
    editor?: LineCellEditor | PasswordCellEditor | NumberCellEditor | MultiLineCellEditor | DropDownCellEditor | MultiCheckCellEditor | SearchCellEditor | DateCellEditor | BTDateCellEditor | ConfigObject | string;
    /**
     * 데이터 셀 우측에 표시할 버튼
     *
     ...
     * @remarks
     *
     * data가 "value,value" 형태로 되어 있는 것을 "label,label" 형태로 보여줄 때 value를 구분하기 위한 구분자
     * 또는 fields의 값들을 구분할 때 사용하는 구분자
     *
     * @defaultValue `,`
     */
    valueSeparator?: string;
 ...
 * DataCellRenderer
 */
declare class DataCellRenderer {}
